...
fun SystemScreen(
    ...
    Column {
        ...
        rows.forEachIndexed { index, row -> ru.nekostul.horizonos.ui.settings.HorizonSettingRow(row, selectedIndex == index, onClick = { onSelect(index) }) }
        Spacer(Modifier.height(16.dp))
        when (selectedIndex) {
            0 -> DateTimeScreen()
            1 -> LanguageSettingsScreen(language, onLanguageSelected)
            2 -> DisplaySettingsScreen()
            3 -> SoundSettingsScreen()
            4 -> AccessibilityScreen()
